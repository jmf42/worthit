...
# ---------------------------------------------------------------------------
# Smartproxy rotates only when the TCP connection closes. The library
# (>= 1.0.1) exposes `prevent_keeping_connections_alive` for that.
class RotatingCloseProxy(GenericProxyConfig):
    def prevent_keeping_connections_alive(self) -> bool:   # noqa: D401
        return True
# ---------------------------------------------------------------------------
...
def _make_proxy_cfg(url: str):
    ...
    if "https_url" in sig.parameters:
        cfg = RotatingCloseProxy(https_url=url, http_url=url)
    else:
        cfg = RotatingCloseProxy(https=url, http=url)
...
def _fetch_transcript_api(video_id: str,
    ...
    session = requests.Session()
    # Smartproxy gives a new residential IP when we close the socket
    session.headers["Connection"] = "close"
    # --- ensure the session actually goes through the selected proxy ---
    if proxy_cfg is not None:
        proxy_url = getattr(proxy_cfg, "https", None) or getattr(proxy_cfg, "https_url", None)
        if proxy_url:
    ...
    for finder in (transcript_list.find_transcript, transcript_list.find_generated_transcript):
        try:
            ...
            # --- call tr.fetch() in a backward‑compatible way ---
            try:
                fetch_sig = inspect.signature(tr.fetch)
                if "session" in fetch_sig.parameters:
                    segments = tr.fetch(session=session)
                else:
                    segments = tr.fetch()
            except TypeError:
                # Fallback for exotic versions that don't like inspect trick
                segments = tr.fetch()

            # --- NEW: convert to raw list[dict] if library ≥ 0.7 provides helper ---
            if hasattr(segments, "to_raw_data"):
                segments = segments.to_raw_data()

            def _extract(seg):
                # youtube‑transcript‑api ≤0.6: dict |   ≥0.7: dataclass with .text
                if isinstance(seg, dict):
                    return seg.get("text", "")
                return getattr(seg, "text", "")

            text = " ".join(_extract(s) for s in segments).strip()
            if text:
                # ensure socket truly closes
                session.close()
                return text
